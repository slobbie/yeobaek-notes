...
- Root `dev`, `build`, and `start` scripts target `@yeobaek/web`; `@yeobaek/admin` exposes a local development script only.
- `apps/*/app/lib/**` and `packages/content/**` are the data boundary. They own Supabase clients, content schemas, analytics contracts, and future repository functions.
