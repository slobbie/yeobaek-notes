  ...
  return <>
    ...
    <main className="site-shell">
      <header className="masthead">
        <a className="brand" href="#top" onClick={reset}><h1>여백의 노트</h1><span>읽고, 쓰고, 생각한 것을 남깁니다.</span></a>
      ...
      <div className="content-grid" id="archive">
        ...
        <section className="archive" aria-labelledby="archive-title">
          ...
          {visiblePosts.length ? visiblePosts.map((post) => <article className="post" key={post.slug}><span className="timeline-dot" aria-hidden="true" /><div className="post-meta"><time dateTime={post.publishedAt}>{formatPostDate(post.publishedAt)}</time><span>{post.category}</span><span>{post.format}</span></div><h3><a href={`#${post.slug}`} onClick={() => track("post_opened", { post_slug: post.slug, category: post.category })}>{post.title}</a></h3><p>{post.excerpt}</p><footer><span>{formatReadingTime(post.readingTimeMinutes)}</span><span>{post.tags.map((tag) => `#${tag}`).join("  ")}</span></footer></article>) : <div className="empty"><p>이 조건에 맞는 기록은 아직 없어요.</p><button type="button" onClick={reset}>필터 초기화</button></div>}
