...
import { formatPostDate, POST_CATEGORIES } from "@yeobaek/content";
...
export function HomeArchive({ posts }) {
  const [category, setCategory] = useState("전체");
  const [query, setQuery] = useState("");
  const [page, setPage] = useState(1);
  const lastTrackedQuery = useRef(null);
  const filteredPosts = useMemo(() => filterPosts(posts, { category, query }), [posts, category, query]);
  ...
  return <>
    ...
    <main className="site-shell">
      ...
      <div className="content-grid" id="archive">
        <aside className="month-mark" aria-label={archivePeriod ? `${archivePeriod.label} 기록` : "아직 발행된 기록 없음"}><strong>{archivePeriod ? <>{archivePeriod.month}<br />{archivePeriod.year}</> : <>—<br />—</>}</strong><p>좋은 생각은<br />시간 속에서<br />더 깊어진다.</p></aside>
        <section className="archive" aria-labelledby="archive-title">
          ...
          {visiblePosts.length ? visiblePosts.map((post) => <article className="post" key={post.slug}><span className="timeline-dot" aria-hidden="true" /><div className="post-meta"><time dateTime={post.publishedAt}>{formatPostDate(post.publishedAt)}</time><span>{post.category}</span></div><h3><Link href={`/posts/${post.slug}`} onClick={() => track("post_opened", { post_slug: post.slug, category: post.category })}>{post.title}</Link></h3><p>{post.excerpt}</p><footer><span>{post.tags.map((tag) => `#${tag}`).join(" · ")}</span></footer></article>) : <div className="empty"><p>{posts.length > 0 ? "이 조건에 맞는 기록은 아직 없어요." : "아직 발행된 기록이 없어요."}</p>{posts.length > 0 && <button type="button" onClick={reset}>필터 초기화</button>}</div>}
